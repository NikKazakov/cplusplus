...
void Cl6Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Text(pDX, IDC_POSX_ECHO, m_MPosX_Echo);
	//DDX_Text(pDX, IDC_POSY_ECHO, m_MPosY_Echo);
...
BEGIN_MESSAGE_MAP(Cl6Dlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_BTN_QUIT, &Cl6Dlg::OnBnClickedBtnQuit)
	ON_BN_CLICKED(IDC_BTN_ADD, &Cl6Dlg::OnBnClickedBtnAdd)
	ON_BN_CLICKED(IDC_BTN_REMOVE, &Cl6Dlg::OnBnClickedBtnRemove)
	ON_BN_CLICKED(IDC_BTN_CONNECT, &Cl6Dlg::OnBnClickedBtnConnect)
	ON_BN_CLICKED(IDC_BTN_HELP, &Cl6Dlg::OnBnClickedBtnHelp)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOVING()
	ON_WM_RBUTTONDOWN()
	ON_BN_CLICKED(IDC_BTN_LOAD, &Cl6Dlg::OnBnClickedBtnLoad)
	ON_BN_CLICKED(IDC_BTN_SAVE, &Cl6Dlg::OnBnClickedBtnSave)
END_MESSAGE_MAP()
...
void Cl6Dlg::InitRouters() {
	CRect rect(0, 0, 0, 0);
	bitmap_addr = "C:\\Users\\plox\\source\\repos\\l6\\l6\\res\\bitmap1.bmp";
...
CRect Cl6Dlg::GetRect(CStatic& wnd) {
	CRect rect;
...
CRect Cl6Dlg::CheckRect(CRect rect) {
	if (rect.left <= 5) {
		rect.left = 6;
...
CPoint Cl6Dlg::CheckPoint(CPoint point) {
	if (point.x <= 5) {
		point.x = 6;
	}
	if (point.x >= 390) {
...
int Cl6Dlg::FindClickedR(CPoint point) {
	CRect rect;
...
void Cl6Dlg::OnMoving(UINT fwSide, LPRECT pRect)
{
...
void Cl6Dlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	switch (Action) {
	case 1: {
		Add(point);
...
void Cl6Dlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (Drag.isDragged) {
		EndDrag(point);
	}
...
void Cl6Dlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	m_ExHelp.ShowWindow(SW_HIDE);
	Action = -1;
...
void Cl6Dlg::OnMouseMove(UINT nFlags, CPoint point)
{
	if (Drag.isDragged) {
		IsDragged(point);
	}
...
void Cl6Dlg::BeginDrag(CPoint point) {
	SetCapture();
...
void Cl6Dlg::IsDragged(CPoint point) {
...
void Cl6Dlg::EndDrag(CPoint point) {
	ReleaseCapture();
	Drag.isDragged = FALSE;
	UpdateData(FALSE);
}
...
void Cl6Dlg::OnBnClickedBtnRemove()
{
...
void Cl6Dlg::OnBnClickedBtnConnect()
{
...
void Cl6Dlg::BeginConnect(CPoint point) {
	SetCapture();
	int a = FindClickedR(point);
	if (a >= 0) {
...
void Cl6Dlg::IsConnecting(CPoint point) {
	DrawConnections();
...
void Cl6Dlg::EndConnect(CPoint point) {
	int a = FindClickedR(point);
...
void Cl6Dlg::OnBnClickedBtnHelp()
{
	if (showing_help) {
		InitHelp();
	}
...
void Cl6Dlg::OnBnClickedBtnQuit()
{
...
void Cl6Dlg::OnBnClickedBtnLoad()
{
	int size = sizeof(Cl6Dlg::m_R) / sizeof(*Cl6Dlg::m_R);
	for (int i = 0; i < size; i++) {
		Remove(i);
	}
	m_Connected.clear();
	std::ifstream f("C:\\Users\\plox\\source\\repos\\l6\\l6\\res\\save.txt");
	if (f.is_open()) {
		int enabled;
		f >> enabled;
...
void Cl6Dlg::OnBnClickedBtnSave()
{
	std::ofstream f("C:\\Users\\plox\\source\\repos\\l6\\l6\\res\\save.txt");
	if (f.is_open()) {
		int size = sizeof(Cl6Dlg::m_R) / sizeof(*Cl6Dlg::m_R);
		int enabled = 0;
		for (int i = 0; i < size; i++) {
